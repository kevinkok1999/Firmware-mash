...
| UI remains responsive during route search/store/energy work | UI/core | non-blocking interaction test under discovery + compaction + energy-state change |
| Queued message visibly transitions to Delivered automatically | UI/reliability | delayed-delivery UI state test |
| Normal UI requires no transport/route/power-electronics tuning | UI/router/energy | user-flow review + settings inspection |
...
UX-004 UI responsive during network/storage/energy work
UX-005 no normal-user transport/route/power tuning requirement
