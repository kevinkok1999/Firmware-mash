...
Energy harvesting is not a transport and does not create a logical mesh hop. The complete contract is in `ENERGY_MANAGEMENT_CONTRACT.md` and ADR 0006.
...
A stock T-Deck Plus must also run the complete stable core **without a microSD card installed** and **without any energy-harvesting hardware installed**. Internal flash is the durable source of truth; PSRAM/RAM are volatile working memory. microSD is optional bulk storage only.
...
- `mog_health`: queue, pool, route, radio and energy health monitoring.
...
- route/neighbor/RF/energy history: RAM/PSRAM by default, reconstructed after reboot;
...
Energy cost comes from normalized EnergyManager/transport observations. HybridRouter does not read PMIC registers or control harvesting hardware directly.

## Reliability

Transport-level success is not end-to-end delivery. `ReliabilityManager` owns application ACKs, bounded retransmission and failover. Delivery classes are planned as BEST_EFFORT, NORMAL and CRITICAL. Critical delivery may use an independent second path only when the airtime and energy budgets allow it.
...
Stable software may use energy state to tune display/GNSS/background discovery, relay willingness and multipath budget. It may not change PacketId, encryption boundaries, delivery truth or bypass ReliabilityManager/AirtimeManager.
...
If ESP-NOW, NAN, RF-assist or an RF-harvest provider fails, the adapter/provider is disabled/degraded and LoRa operation continues. Every queue, peer table, route set and packet pool must have an explicit overflow policy and metric. Silent failure is forbidden.
...
Implausible energy samples must be rejected/low-confidence; an energy provider may not directly disable the required LoRa backbone.