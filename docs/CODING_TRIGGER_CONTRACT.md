...
Provide one authoritative entrypoint for Firmware-mash implementation. The trigger is a contract for the coding process, not permission to bypass evidence gates.

## Current decision

`CODING_START=GO`

Coding has started on `develop`. Current status and phase boundaries are recorded in `CODING_START_READY.md` and `THREE_PHASE_IMPLEMENTATION_PLAN.md`.

Promotion remains fail-closed: source may be developed in isolation at a higher phase when useful for host testing, but it may not be integrated/promoted around an unproven lower phase.
...
Concrete/core       = origin-qualified message key + PacketId + events + lifecycle + reboot-safe time
Utilities           = EnergyManager + health primitives
Structural frame    = LoRa transport + AirtimeManager + HybridRouter
Safety systems      = ReliabilityManager + E2E ACK/retry + dedup + delayed delivery
...
- `docs/BASELINE_APPROVED` exists and passes marker checks;
...
Those conditions are now satisfied for coding start. Current GitHub-hosted runner/scheduling failure is tracked in issue #19; it blocks repository-local target-build evidence but is not a firmware compile result.

## Authoritative instruction

> Execute Firmware-mash from the current repository contracts using a strict foundation-first construction strategy. Do not redesign settled architecture. Follow `THREE_PHASE_IMPLEMENTATION_PLAN.md`, `ONE_SHOT_IMPLEMENTATION_RUNBOOK.md` and `IMPLEMENTATION_BLUEPRINT.md` in dependency order. Never build a higher-level feature to compensate for an unproven lower-level dependency. Implement missing approved `mog_` code rather than omitting capabilities. After each major layer, compile, run mapped tests, run the LoRa-stable regression when available, record resource/evidence impact, repair ordinary failures in-loop, and continue. Stop promotion only for a proven measurement/hardware/toolchain/access blocker. Never invent hardware validation, custom crypto or release evidence.
...
8. three-phase plan / implementation blueprint / runbook / roadmap;
...
3. `mog_core`: origin-qualified logical identity, durable PacketId, lifecycle and time;
4. `mog_events`: bounded event ownership and overflow/resync behavior;
5. `mog_conversation` / `mog_messaging`: same-chat identity/order/presentation;
6. EnergyManager + core health/runtime-safety primitives;
7. LoRa + AirtimeManager;
8. HybridRouter LoRa-only seam;
9. ReliabilityManager + dedup + sender delayed delivery;
10. bounded multipath/route scoring;
11. optional custody core/store-carry state machine on the proven LoRa/storage/reliability foundation;
12. ESP-NOW Normal/LR adapter + 2.4-GHz RadioScheduler;
13. IP backhaul + gateway federation;
14. cross-transport recovery/event integration for queued/custody-held messages;
15. health/metrics/controller checks;
16. smartphone-like UI integration;
17. optional Ambient RF provider seam;
18. optional lab transports/hardware providers;
19. release-engineering handoff.

Why custody precedes extra transports: its durability/ownership semantics must first be proven without mixing in Wi-Fi/ESP-NOW complexity. Later transports simply become additional forwarding opportunities for the same custody-held logical message.

## Stage admission rule

Before target integration/promotion of stage N+1, stage N must have:

- compiling target/configuration where target tooling is available;
...
-> compile host/unit target available for that layer
-> run mapped unit/simulator/fault tests
-> run impacted integration tests
-> run CFG-LORA-STABLE regression when available
-> inspect resource delta
-> inspect architecture/security boundary
-> repair failure at its owning layer
-> record evidence
-> admit next integration layer
```
...
- one durable MessageStore authority;
- logical message identity is `(origin, PacketId)`; the PacketId is stable across transports/retries/custody;
...
- critical event overflow requires explicit resync/error behavior, never silent loss;
...
Phase 2 tests: `test/run_phase2_tests.sh`.
Phase 3 tests: `test/run_phase3_tests.sh`.
Release packaging: `scripts/release-package.sh`.

All are intentionally fail-closed.

## Completion definition

The coding process is complete only when every targeted capability is either implemented with mapped passing evidence for its tier or explicitly blocked by a measured external/hardware/toolchain fact while the stable core remains working.
